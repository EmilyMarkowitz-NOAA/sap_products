...
/* STRATA AREA, FOR THE plusnw EBS SHELF WHERE STRATA <= 90 (BOTTOM DEPTH < 200M) */
...
REM REMEMBER HERE TO EXCLUDE DATA PRIOR TO 1987 SINCE PLUSNW STATIONS WERE RARELY CONDUCTED
...
where f.stratum=r.stratum and year>=1987;
...
drop table bttemp_ebs_plusnw_lt200m;
drop view bttemp_ebs_plusnw_lt200m;
create table bttemp_ebs_plusnw_lt200m as
select year, sum(gtempwgt) avgbsbt, sum(stempwgt) avgbsst from geartemp_subarea 
group by year;

grant select on bttemp_ebs_plusnw_lt200m to public;

select * from bttemp_ebs_plusnw_lt200m order by year;
/* THIS SCRIPT CALCULATES AVERAGE BOTTOM AND SURFACE TEMPERATURE, WEIGHTED */
/* STRATA AREA, FOR THE STANDARD EBS SHELF WHERE STRATA <= 62 (BOTTOM DEPTH < 200M) */
...
and stratum in (10,20,31,32,41,42,43,50,61,62);
...
drop table bttemp_ebs_standard_lt200m;
drop view bttemp_ebs_standard_lt200m;
create table bttemp_ebs_standard_lt200m as
select year, sum(gtempwgt)avgbsbt, sum(stempwgt)avgbsst from geartemp_subarea 
group by year;

grant select on bttemp_ebs_standard_lt200m to public;

select * from bttemp_ebs_standard_lt200m order by year;

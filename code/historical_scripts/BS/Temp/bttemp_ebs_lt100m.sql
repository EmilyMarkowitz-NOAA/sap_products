...
/* STRATA AREA, FOR THE STANDARD EBS SHELF WHERE STRATA <= 43 (BOTTOM DEPTH < 100M) */
...
AND A.STRATUM IN (10,20,31,32,41,42,43)
...
and stratum in (10,20,31,32,41,42,43);
...
drop table bttemp_ebs_lt100m;
drop view bttemp_ebs_lt100m;
create table bttemp_ebs_lt100m as
select year, sum(gtempwgt) avgbsbt, sum(stempwgt) avgbsst from geartemp_subarea 
group by year;

grant select on bttemp_ebs_lt100m to public;

select * from bttemp_ebs_lt100m order by year;
